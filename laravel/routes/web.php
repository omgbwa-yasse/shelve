    ...
    */
    ...
    Route::get('count-unread', [MailController::class, 'countUnread'])->name('mails.count-unread');
        ...
        */
        ...
        /*
            Email — vraie boîte de messagerie IMAP/SMTP (distincte du courrier
            administratif ci-dessus). Voir MailboxController/EmailAccountController.
        */
        Route::prefix('email')->name('mails.email.')->middleware(['email.enabled'])->group(function () {
            Route::get('/', [\App\Http\Controllers\MailboxController::class, 'inbox'])->name('inbox');
            Route::get('/sent', [\App\Http\Controllers\MailboxController::class, 'sent'])->name('sent');
            Route::get('/compose', [\App\Http\Controllers\MailboxController::class, 'compose'])->name('compose');
            Route::post('/send', [\App\Http\Controllers\MailboxController::class, 'send'])->name('send');

            // Doit être déclaré AVANT `/{emailMessage}` : sinon un GET /mails/email/tags
            // matche le wildcard en premier et tente un binding EmailMessage::find('tags') → 404.
            Route::resource('tags', \App\Http\Controllers\EmailTagController::class)
                ->only(['index', 'store', 'update', 'destroy'])
                ->names('tags.manage')
                ->parameters(['tags' => 'emailTag']);

            Route::get('/{emailMessage}', [\App\Http\Controllers\MailboxController::class, 'show'])->name('show');
            Route::get('/{emailMessage}/attachments/{emailAttachment}', [\App\Http\Controllers\MailboxController::class, 'downloadAttachment'])->name('attachments.download');
            Route::delete('/{emailMessage}', [\App\Http\Controllers\MailboxController::class, 'destroy'])->name('destroy');
            Route::post('/{emailMessage}/toggle-flag', [\App\Http\Controllers\MailboxController::class, 'toggleFlag'])->name('toggle-flag');
            Route::post('/{emailMessage}/tags', [\App\Http\Controllers\MailboxController::class, 'attachTag'])->name('tags.attach');
            Route::delete('/{emailMessage}/tags/{emailTag}', [\App\Http\Controllers\MailboxController::class, 'detachTag'])->name('tags.detach');
        });
    ...
    Route::prefix('settings')->group(function () {
        ...
        // Comptes de messagerie (IMAP/SMTP) — voir App\Http\Controllers\MailboxController.
        Route::resource('email-accounts', \App\Http\Controllers\Settings\EmailAccountController::class)
            ->except(['show'])
            ->names('settings.email-accounts');
        Route::post('email-accounts/{emailAccount}/test-connection', [\App\Http\Controllers\Settings\EmailAccountController::class, 'testConnection'])->name('settings.email-accounts.test-connection');
        Route::post('email-accounts/{emailAccount}/sync', [\App\Http\Controllers\Settings\EmailAccountController::class, 'syncNow'])->name('settings.email-accounts.sync');
        Route::post('email-accounts/{emailAccount}/toggle-active', [\App\Http\Controllers\Settings\EmailAccountController::class, 'toggleActive'])->name('settings.email-accounts.toggle-active');
        Route::post('email/toggle', [\App\Http\Controllers\Settings\EmailAccountController::class, 'toggleModule'])->name('settings.email.toggle');
