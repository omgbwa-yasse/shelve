...
<div class="container py-3">
    <nav aria-label="breadcrumb" class="mb-3">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active" aria-current="page">{{ __('Edit contact') }}</li>
        </ol>
    </nav>
    ...
    <div class="card shadow-sm">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('organisations.contacts.update', [$organisation, $contact]) }}" id="contactForm">
                @csrf
                @method('PUT')
                <div class="row g-3">
                    <div class="col-md-3">
                        ...
                        <select name="type" id="contactType" class="form-select" required>
                            @foreach(['email','telephone','gps','fax','code_postal','adresse'] as $t)
                                <option value="{{ $t }}" @selected($contact->type === $t)>{{ $t }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="form-label" for="contactValue">{{ __('Value') }}</label>
                        <textarea name="value" id="contactValue" class="form-control" rows="2" required>{{ $contact->value }}</textarea>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label" for="contactLabel">{{ __('Label') }}</label>
                        <input type="text" id="contactLabel" name="label" class="form-control" maxlength="190" value="{{ $contact->label }}">
                    </div>
                    <div class="col-12">
                        <label class="form-label" for="contactNotes">{{ __('Notes') }}</label>
                        <textarea name="notes" id="contactNotes" class="form-control" rows="2">{{ $contact->notes }}</textarea>
                    </div>
                </div>
