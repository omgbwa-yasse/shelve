 ...
 */
class RecordSupportApiTest extends TestCase
{
    use DatabaseTransactions, WithPermissions;

    /**
     * Préfixe des permissions pour cette ressource (voir RecordSupportPolicy).
     *
     ...
     * 'law_*', pas 'law_article_*'). Ouvrir app/Policies/RecordSupportPolicy.php pour
     * confirmer avant de faire confiance à ce test.
     */
    private const PERMISSIONS = ['record_support'];
    ...
    {
        $this->getJson('/api/v1/record-supports')->assertStatus(401);
    }
    ...
    public function test_index_sans_permission_est_refuse(): void
    {
        ...
        $this->actingAs($user, 'sanctum')
            ->getJson('/api/v1/record-supports')
            ->assertStatus(403);
    }
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/record-supports', [])
            ->assertStatus(422);
    }
