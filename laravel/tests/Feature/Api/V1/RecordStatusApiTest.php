 ...
 */
class RecordStatusApiTest extends TestCase
{
    use DatabaseTransactions, WithPermissions;

    /**
     * Préfixe des permissions pour cette ressource (voir RecordStatusPolicy).
     *
     ...
     * 'law_*', pas 'law_article_*'). Ouvrir app/Policies/RecordStatusPolicy.php pour
     * confirmer avant de faire confiance à ce test.
     */
    private const PERMISSIONS = ['record_status'];
    ...
    {
        $this->getJson('/api/v1/record-statuses')->assertStatus(401);
    }
    ...
    public function test_index_sans_permission_est_refuse(): void
    {
        ...
        $this->actingAs($user, 'sanctum')
            ->getJson('/api/v1/record-statuses')
            ->assertStatus(403);
    }
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/record-statuses', [])
            ->assertStatus(422);
    }
