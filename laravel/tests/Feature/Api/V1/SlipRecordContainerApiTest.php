...
/**
 * D04 — association document↔contenant (pivot `slip_record_container`, org-scopée par
 * héritage du slip parent, R03). Portage finalisé le 2026-08-04 : la ressource est
 * imbriquée sous `/slips/{slip}/records/{record}/containers` (motif D03).
 */
class SlipRecordContainerApiTest extends TestCase
{
    use DatabaseTransactions, WithPermissions;

    private const PERMISSIONS = ['slip_record_container'];
    ...
    {
        $this->getJson('/api/v1/slips/1/records/1/containers')->assertStatus(401);
    }
    ...
    public function test_index_sans_permission_est_refuse(): void
    {
        ...
        $this->actingAs($user, 'sanctum')
            ->getJson("/api/v1/slips/{$slipRecord->slip_id}/records/{$slipRecord->id}/containers")
            ->assertStatus(403);
    }
    ...
    public function test_store_valide_ses_entrees(): void
    {
        ...
        $this->actingAs($this->user, 'sanctum')
            ->postJson("/api/v1/slips/{$slipRecord->slip_id}/records/{$slipRecord->id}/containers", [])
            ->assertStatus(422);
    }
}
