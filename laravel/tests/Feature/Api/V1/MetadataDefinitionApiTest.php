 ...
 */
class MetadataDefinitionApiTest extends TestCase
{
    use DatabaseTransactions, WithPermissions;

    /**
     * Préfixe des permissions pour cette ressource (voir MetadataDefinitionPolicy).
     *
     ...
     * 'law_*', pas 'law_article_*'). Ouvrir app/Policies/MetadataDefinitionPolicy.php pour
     * confirmer avant de faire confiance à ce test.
     */
    private const PERMISSIONS = ['metadata_definition'];
    ...
    {
        $this->getJson('/api/v1/metadata-definitions')->assertStatus(401);
    }
    ...
    public function test_index_sans_permission_est_refuse(): void
    {
        ...
        $this->actingAs($user, 'sanctum')
            ->getJson('/api/v1/metadata-definitions')
            ->assertStatus(403);
    }
    ...
    {
        $this->actingAs($this->user, 'sanctum')
            ->postJson('/api/v1/metadata-definitions', [])
            ->assertStatus(422);
    }
