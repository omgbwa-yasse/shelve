...
class WorkplaceController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'code' => ['nullable', 'string', 'max:50', 'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/i', 'unique:workplaces,code'],
        ]);
        ...
        try {
            $workplace = Workplace::create([
                ...$validated,
                // Le code saisi à la création devient le slug d'accès
                // (`/workplace/{code}`) ; sinon génération automatique WP-YYYY-NNNN.
                'code' => isset($validated['code']) && $validated['code'] !== ''
                    ? mb_strtolower(trim($validated['code']))
                    : $this->generateWorkplaceCode(),
    ...
    public function update(Request $request, Workplace $workplace)
    {
        ...
        $validated = $request->validate([
            ...
            'max_members' => 'nullable|integer|min:1',
            'max_storage_mb' => 'nullable|integer|min:1',
            'code' => ['nullable', 'string', 'max:50', 'regex:/^[a-z0-9]+(?:-[a-z0-9]+)*$/i', Rule::unique('workplaces', 'code')->ignore($workplace)],
        ]);
