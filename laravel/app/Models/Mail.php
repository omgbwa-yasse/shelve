...
class Mail extends Model
{
    ...
    */
     ...
     */
    /**
     * Limite une liste aux courriers qui concernent l'organisation courante,
     * y compris les cotations multi-directions.
     */
    public function scopeInOrganisation($query, int $organisationId)
    {
        return $query->where(function ($q) use ($organisationId) {
            $q->where('sender_organisation_id', $organisationId)
              ...
              ->orWhereHas('cotations', function ($cotation) use ($organisationId) {
                  $cotation->where('organisation_id', $organisationId);
              });
        });
    }
