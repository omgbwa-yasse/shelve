...
class Record extends Model
{
    ...
    protected $casts = [
        'metadata' => 'array',
        'is_workplace_folder' => 'boolean',
        'is_workplace_shared' => 'boolean',
        'unavailable' => 'boolean',
    ...
    */
    ...
    /**
     * Espace de travail propriétaire de la notice (bibliothèque Documents).
     */
    ...
    */
    ...
    /**
     * Notices de la bibliothèque Documents d'un espace de travail.
     */
    public function scopeInWorkplace(Builder $query, int $workplaceId): Builder
    {
        return $query->where($this->getTable().'.workplace_id', $workplaceId);
    }

    public function scopeWorkplaceFolder(Builder $query): Builder
    {
        return $query->where('is_workplace_folder', true);
    }

    public function scopeWorkplaceDocument(Builder $query): Builder
    {
        return $query->where('is_workplace_folder', false);
    }

    /**
     * Visibilité module Records : un document du workplace n'y est visible QUE
     * s'il a été partagé (`is_workplace_shared`). Les notices ordinaires (sans
     * workplace) restent visibles. Le transfert vers Records pose `workplace_id`
     * à null, ce qui le sort de ce filtre par la branche `whereNull`.
     */
    public function scopeVisibleToRecordsModule(Builder $query): Builder
    {
        return $query->where(fn ($q) => $q
            ->whereNull('workplace_id')
            ->orWhere('is_workplace_shared', true));
    }
