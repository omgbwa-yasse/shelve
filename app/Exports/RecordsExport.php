...
class RecordsExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    {
        // Les champs descriptifs ISAD(G) vivent désormais dans `metadata`
        // (MetadataDefinition système) plutôt qu'en colonnes directes sur `records`.
        $meta = fn (string $field) => $record->getMetadataValue($field);
        ...
        return [
            ...
            $meta('biographical_history'),
            $meta('archival_history'),
            ...
            $meta('reproduction_conditions'),
            $meta('language_material'),
            $meta('characteristic'),
            $meta('finding_aids'),
            $meta('location_original'),
            $meta('location_copy'),
            $meta('related_unit'),
            $meta('publication_note'),
            $meta('note'),
            $meta('archivist_note'),
            $meta('rule_convention'),
