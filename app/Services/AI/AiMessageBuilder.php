...
class AiMessageBuilder
{
    ...
    public function buildMailSummaryMessages($mail, string $systemPrompt = ''): array
    {
        ...
        foreach ($mail->attachments as $attachment) {
            ...
            // Priorité 1: Contenu extrait déjà disponible
            if (!empty($attachment->content_text)) {
                $content = mb_substr($attachment->content_text, 0, 5000);
                $doc['content'] = $content;
                $attachmentsContent[] = $attachLabel . $attachment->name . "\nContenu extrait:\n" . $content;
            }
            // Priorité 2: Fichiers texte à lire directement
            elseif (str_starts_with($mime, 'text/') && file_exists($filePath)) {
                ...
                if ($text) {
                    ...
                    $attachmentsContent[] = $attachLabel . $attachment->name . " (fichier texte non lisible)";
                }
            }
            // Priorité 3: PDFs - tentative d'extraction
            elseif ($mime === 'application/pdf' && file_exists($filePath)) {
                $extractedText = $this->extractPdfText($filePath);
                if ($extractedText) {
                    $content = mb_substr($extractedText, 0, 5000);
                    $doc['content'] = $content;
                    $attachmentsContent[] = $attachLabel . $attachment->name . " (PDF)\nContenu extrait:\n" . $content;
                } else {
                    $attachmentsContent[] = $attachLabel . $attachment->name . " (PDF - extraction impossible)";
                }
            }
            // Priorité 4: Images - description du type seulement
            elseif (str_starts_with($mime, 'image/')) {
                $attachmentsContent[] = $attachLabel . $attachment->name . " (Image - " . $mime . ")";
            }
            // Priorité 5: Autres types de fichiers
            else {
                $attachmentsContent[] = $attachLabel . $attachment->name . " (Type: " . $mime . ")";
            }
        ...
        // Amélioration du prompt système pour traiter les fichiers joints
        $defaultSystemPrompt = "Tu es un assistant spécialisé dans l'analyse et le résumé de courriers administratifs. " .
            "Analyse le courrier et ses pièces jointes pour produire un résumé professionnel et des mots-clés archivistiques pertinents. " .
            "Si des pièces jointes contiennent du texte, intègre leur contenu dans ton analyse pour enrichir le résumé.";
    ...
    {
        try {
            // Méthode 1: Utiliser smalot/pdfparser si disponible
            ...
            // Méthode 2: Utiliser pdftotext si disponible (Linux/Unix)
            ...
            // Méthode 3: Lecture basique du PDF (très limitée)
            $content = @file_get_contents($filePath);
            if ($content) {
                // Recherche de texte simple dans le PDF
            ...
            \Illuminate\Support\Facades\Log::warning('Erreur extraction PDF: ' . $e->getMessage());
        }
