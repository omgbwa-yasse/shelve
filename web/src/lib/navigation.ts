...
export function findActiveDomain(pathname: string | null | undefined): NavDomain | undefined {
  ...
  // WorkPlace : les espaces vivent désormais sur `/workplace/{code}` (slug, ex.
  // `/workplace/rh`, `/workplace/sia2019`). On rattache ces chemins au domaine
  // WorkPlaces (`/workplaces`) pour que le rail et le sous-menu restent actifs.
  if (pathname === '/workplace' || pathname.startsWith('/workplace/')) {
    return navigation.find((domain) => domain.key === 'workplaces');
  }
...
export const navigation: NavDomain[] = [
  ...
  // ------------------------------------------------------------------
  {
    ...
    icon: 'mails',
    items: [
      { key: 'mails-email-inbox', group: 'Email', label: 'Boîte de réception', href: '/mails/email', icon: 'inbox' },
      { key: 'mails-email-sent', group: 'Email', label: 'Envoyés', href: '/mails/email/sent', icon: 'send' },
      { key: 'mails-email-compose', group: 'Email', label: 'Composer', href: '/mails/email/compose', icon: 'plusSquare' },
      { key: 'mails-email-tags', group: 'Email', label: 'Étiquettes', href: '/mails/email/tags', icon: 'tags' },
      { key: 'mails-email-accounts', group: 'Email', label: 'Paramètres', href: '/mails/email/accounts', icon: 'settings' },
  ...
  // ------------------------------------------------------------------
  {
    ...
    items: [
      ...
      { key: 'projects-risks', group: 'Suivi global', label: 'Tous les risques', href: '/projects/risks', icon: 'alertTriangle' },
