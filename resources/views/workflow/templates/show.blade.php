...
<div class="container-fluid">
    @php
        $configurationData = $template->configuration;
        if (is_string($configurationData)) {
            $decoded = json_decode($configurationData, true);
            $configurationData = is_array($decoded) ? $decoded : [];
        } elseif (!is_array($configurationData)) {
            $configurationData = [];
        }
    @endphp
    <div class="row mb-4">
    ...
    <div class="row">
            ...
            <div class="card shadow-sm mb-4">
                ...
                <div class="card-body">
                    <div class="row text-center">
                        <div class="col-6 mb-3"><div class="h4">{{ $template->steps->count() }}</div><div class="text-muted small">{{ __('Étapes DB') }}</div></div>
                        <div class="col-6 mb-3"><div class="h4">{{ $template->instances->count() }}</div><div class="text-muted small">{{ __('Instances') }}</div></div>
                        <div class="col-12"><div class="h5 {{ !empty($configurationData) ? 'text-success' : 'text-muted' }}">{{ !empty($configurationData) ? count($configurationData) : 0 }}</div><div class="text-muted small">{{ !empty($configurationData) ? __('Étapes JSON') : __('Aucune config JSON') }}</div></div>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-lg-8">
            <ul class="nav nav-tabs" id="templateTabs" role="tablist">
                ...
                <li class="nav-item" role="presentation"><button class="nav-link" id="configuration-tab" data-bs-toggle="tab" data-bs-target="#configuration" type="button" role="tab"><i class="bi bi-code-square me-1"></i>{{ __('Configuration JSON') }}</button></li>
            </ul>
                ...
                <div class="tab-pane fade" id="configuration" role="tabpanel">
                    <div class="card shadow-sm border-top-0">
                        <div class="card-header bg-light d-flex justify-content-between align-items-center">
                            ...
                            <div class="btn-group btn-group-sm">
                                <button type="button" class="btn btn-primary" onclick="addConfigurationStep()"><i class="bi bi-plus-lg me-1"></i>{{ __('Ajouter une étape') }}</button>
                                <button type="button" class="btn btn-success" onclick="validateConfiguration()"><i class="bi bi-check-circle me-1"></i>{{ __('Valider') }}</button>
                                <button type="button" class="btn btn-info" onclick="saveConfiguration()"><i class="bi bi-save me-1"></i>{{ __('Sauvegarder') }}</button>
                                <div class="btn-group btn-group-sm">
                                    <button type="button" class="btn btn-outline-secondary dropdown-toggle" data-bs-toggle="dropdown"><i class="bi bi-gear me-1"></i>{{ __('Actions') }}</button>
                                    <ul class="dropdown-menu">
                                        <li><a class="dropdown-item" href="#" onclick="importFromSteps()"><i class="bi bi-arrow-down-circle me-2"></i>{{ __('Importer depuis les étapes') }}</a></li>
                                        <li><a class="dropdown-item" href="#" onclick="exportConfiguration()"><i class="bi bi-arrow-up-circle me-2"></i>{{ __('Exporter JSON') }}</a></li>
                                        <li><hr class="dropdown-divider"></li>
                                        <li><a class="dropdown-item text-warning" href="#" onclick="resetConfiguration()"><i class="bi bi-arrow-clockwise me-2"></i>{{ __('Réinitialiser') }}</a></li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                            ...
                            @if(!empty($configurationData))
                                <div class="mb-4">
                                    ...
                                    <div id="current-configuration" class="bg-light p-3 rounded">
                                        <pre class="mb-0"><code id="current-config-json">{{ json_encode($configurationData, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</code></pre>
                                    </div>
                                </div>
                            @endif
                            <div class="configuration-editor">
                                <h6>{{ __('Éditeur de configuration:') }}</h6>
                                <div id="configuration-steps" class="mb-3"></div>
                                <div class="d-flex gap-2 mb-3">
                                    <button type="button" class="btn btn-outline-primary btn-sm" onclick="importFromSteps()"><i class="bi bi-arrow-down-circle me-1"></i>{{ __('Importer depuis les étapes') }}</button>
                                    <button type="button" class="btn btn-outline-secondary btn-sm" onclick="resetConfiguration()"><i class="bi bi-arrow-clockwise me-1"></i>{{ __('Réinitialiser') }}</button>
                                    <button type="button" class="btn btn-outline-info btn-sm" onclick="previewJSON()"><i class="bi bi-eye me-1"></i>{{ __('Aperçu JSON') }}</button>
                                </div>
                                <div id="json-preview" class="d-none">
                                    <h6>{{ __('Aperçu JSON:') }}</h6>
                                    <div class="bg-dark text-light p-3 rounded"><pre class="mb-0"><code id="preview-json-content"></code></pre></div>
                                </div>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    // Variables globales (configuration normalisée en tableau côté serveur)
    let currentConfiguration = @json($configurationData ?? []);
    ...
    window.resetConfiguration = function() {
        if (confirm('Réinitialiser la configuration ? Toutes les modifications non sauvegardées seront perdues.')) {
            currentConfiguration = @json($configurationData ?? []);
            loadConfigurationEditor();
