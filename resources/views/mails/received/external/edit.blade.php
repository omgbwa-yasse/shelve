...
@section('content')
    <div class="container">
        <h1 class="mb-4">Modifier Courrier reçu externe</h1>
        @if ($errors->any())
        ...
        <form action="{{ route('mails.received.external.update', $mail->id) }}" method="POST" class="needs-validation" enctype="multipart/form-data" novalidate>
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-4 mb-3">
                    ...
                    <select name="sender_type" id="sender_type" class="form-select" required>
                        ...
                        <option value="external_contact" {{ old('sender_type', $mail->sender_type) == 'external_contact' ? 'selected' : '' }}>Contact externe</option>
                        <option value="external_organization" {{ old('sender_type', $mail->sender_type) == 'external_organization' ? 'selected' : '' }}>Organisation externe</option>
                        <option value="organisation" {{ old('sender_type', $mail->sender_type) == 'organisation' ? 'selected' : '' }}>Organisation</option>
                    </select>
                </div>
            </div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="external_sender_id" id="external_sender_id" class="form-select">
                            ...
                            @foreach($externalContacts as $contact)
                                <option value="{{ $contact->id }}" {{ old('external_sender_id', $mail->external_sender_id) == $contact->id ? 'selected' : '' }}>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="external_sender_organization_id" id="external_sender_organization_id" class="form-select">
                            ...
                            @foreach($externalOrganizations as $organization)
                                <option value="{{ $organization->id }}" {{ old('external_sender_organization_id', $mail->external_sender_organization_id) == $organization->id ? 'selected' : '' }}>
                                    {{ $organization->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select name="sender_organisation_id" id="sender_organisation_id" class="form-select">
                            ...
                            @foreach($senderOrganisations as $organization)
                                <option value="{{ $organization->id }}" {{ old('sender_organisation_id', $mail->sender_organisation_id) == $organization->id ? 'selected' : '' }}>
                                    {{ $organization->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="d-flex justify-content-between">
                <a href="{{ route('mails.received.external.show', $mail->id) }}" class="btn btn-secondary">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function toggleSenderSections() {
                const selectedType = senderTypeSelect.value;
            ...
            senderTypeSelect.addEventListener('change', toggleSenderSections);
