...
<div class="container-fluid">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h1 class="h3">Profils de métadonnées - {{ $folderType->name }}</h1>
            <p class="text-muted">Gérer les champs de métadonnées pour ce type de dossier</p>
        </div>
        ...
        <div class="col-lg-8">
            <div class="card">
                <div class="card-header bg-light d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">Métadonnées associées ({{ $folderType->metadataProfiles->count() }})</h5>
                </div>
                <div class="card-body">
                    @if($folderType->metadataProfiles->count() > 0)
                                    ...
                                    @foreach($folderType->metadataProfiles as $profile)
                                        <tr class="sortable-row" data-profile-id="{{ $profile->id }}">
                                            ...
                                            <td class="text-end">
                                                ...
                                                <form action="{{ route('settings.folder-types.metadata-profiles.destroy', [$folderType, $profile]) }}"
                        ...
                        <div class="mt-3">
                            <form method="POST" action="{{ route('settings.folder-types.metadata-profiles.bulk-update', $folderType) }}" id="sortForm">
                                @csrf
            ...
            <div class="card">
                ...
                <div class="card-body">
                    @if($availableDefinitions->count() > 0)
                        <form action="{{ route('settings.folder-types.metadata-profiles.store', $folderType) }}"
                              method="POST">
                            @csrf
...
if (editModal) {
    editModal.addEventListener('show.bs.modal', function(event) {
        ...
        if (profileStr && definitionStr) {
            ...
            editForm.action = `{{ route('settings.folder-types.metadata-profiles.index', $folderType) }}/${profile.id}`;
